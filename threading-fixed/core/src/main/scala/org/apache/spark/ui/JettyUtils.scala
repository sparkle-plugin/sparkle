...
private[spark] object JettyUtils extends Logging {
  ...
  def startJettyServer(
    ...
    def connect(currentPort: Int): (Server, Int) = {
      ...
      val min = 1
      val MAX_NUMBER_HTTP_SERVER_THREADS = conf.getInt("spark.jetty.httpserver.numThreads", 10)
      logInfo("max number of jetty http server threads: " + MAX_NUMBER_HTTP_SERVER_THREADS)
      pool.setMinThreads(min)
      pool.setMaxThreads(MAX_NUMBER_HTTP_SERVER_THREADS)
