...
private[spark] abstract class RestSubmissionServer(
  ...
  private def doStart(startPort: Int): (Server, Int) = {
    ...
    // force max number of the threads
    val min = 1
    val MAX_NUMBER_HTTP_SERVER_THREADS = masterConf.getInt("spark.jetty.httpserver.numThreads", 10)
    logInfo("max number of rest server threads: " + MAX_NUMBER_HTTP_SERVER_THREADS)
    threadPool.setMinThreads(min)
    threadPool.setMaxThreads(MAX_NUMBER_HTTP_SERVER_THREADS)
    ...
    // to accomodate number of acceptors same as the specified httpserver threads
    val connectors = server.getConnectors()
    val MAX_NUMBER_ACCEPTORS = MAX_NUMBER_HTTP_SERVER_THREADS
    if (connectors(0).isInstanceOf[AbstractConnector]) {
       val cconnector = connectors(0).asInstanceOf[AbstractConnector]
       cconnector.setAcceptors(MAX_NUMBER_ACCEPTORS)
    }
    logInfo("max number of rest server acceptors: " + MAX_NUMBER_ACCEPTORS)
