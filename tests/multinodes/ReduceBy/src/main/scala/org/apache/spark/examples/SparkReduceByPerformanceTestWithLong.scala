 ...
 */
object SparkReduceByPerformanceTestWithLong {
  ...
  def main(args: Array[String]) {
    ...
    val pairs1 = sc.parallelize(0 until numMappers, numMappers).zipWithIndex.flatMap {
      ...
      var arr1 = new Array[(Long, Double)](numKVPairs)
      for (i <- 0 until numKVPairs) {
        val doubleArr = ranGen.nextDouble()
        arr1(i) =( (ranGen.nextDouble() *upperRange).toLong, doubleArr)
      }
      arr1
    ...
    val pairsRdd  = pairs1.reduceByKey(_+_, numReducers)
    //to retest with count()
    ...
    val sumValues= pairsRdd.map(x => x._2).sum

    val pairsCountAfterGroup=pairsRdd.count()
    val pairs1Count = pairs1.count()

    println("the reduced pairs have the count: " + pairsCountAfterGroup +" vs."
                      + pairs1Count)
    println("reduceby time is (milliseconds): " + (timeSort1-timeSort0))
