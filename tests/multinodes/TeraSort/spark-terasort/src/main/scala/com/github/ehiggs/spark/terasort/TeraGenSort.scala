  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 3) {
      ...
      println("DRIVER_MEMORY=[mem] spark-submit " +
        "com.github.ehiggs.spark.terasort.TeraGenSort " +
        "spark-terasort-1.0-SNAPSHOT-with-dependencies.jar " +
        "[output-size] [part-num] [output-number-of-partitions] [output-directory]")
      println(" ")
      println("Example:")
      println("DRIVER_MEMORY=50g spark-submit " +
        "com.github.ehiggs.spark.terasort.TeraGenSort " +
        "spark-terasort-1.0-SNAPSHOT-with-dependencies.jar " +
        "100G 100 file:///scratch/username/terasort_out")
      System.exit(0)
    }
