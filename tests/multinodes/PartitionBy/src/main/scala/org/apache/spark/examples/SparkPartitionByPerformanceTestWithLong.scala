...
/** simple multi-processed partitionby test with long key*/
object SparkPartitionByPerformanceTestWithLong {
  ...
  def main(args: Array[String]) {
    ...
    //retested it with count()
    //partitionedByRdd.foreachPartition( x=> {} )
    ...
    println("the groupped pairs have the count: " + pairsCountAfterGroup +" vs."
                      + pairs1Count)
    println("partitionby time is (milliseconds): " + (timeSort1-timeSort0))
